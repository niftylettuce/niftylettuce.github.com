...
module.exports = function(grunt){
  ...
  grunt.initConfig({
      dir: './_deploy'
    , gitUrl: 'git@github.com:niftylettuce/niftylettuce.github.com.git'
  })
  ...
  grunt.registerTask('deploy', 'Deploy content to github', function () {
    ...
    function resetRepo(err) {
      ...
      log('Building site')
      exec('node_modules/.bin/wintersmith build -v -o ' + config('dir'), builtDeploy)
    }
    ...
    function gotLastCommit(err, stdout) {
      ...
      if (process.env.NODE_ENV === 'travis-ci') {
        ...
        dirExec('git config user.email "travis-ci@niftylettuce.com" && git config user.name "Travis CI Bot"', makeCommit.bind(null, stdout))
      } else {
        makeCommit(stdout)
      }
    }
